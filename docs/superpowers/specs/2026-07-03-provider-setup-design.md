...
cc-app 启动时的无感接线（`ccsetup.rs`）只覆盖 Claude Code。codex/kimi 会话能上板，靠的是本机开发期**手工写**的配置（`~/.codex/hooks.json`、`~/.kimi-code/config.toml [[hooks]]`），且硬编码指向仓库 dev 构建产物——新装用户 codex/kimi 完全上不了板，本机配置也会因 `cargo clean`/挪目录而静默失效。
...
| C. 下沉为 `cc-reporter install` 子命令 | app 与手动脚本共用 | 改变 reporter「一次性 hook 进程」职责定位，sidecar 调用参数化复杂；YAGNI，弃 |
...
claude 的 hook command 是**裸路径**（`"<path>"`），codex/kimi 的是**带参数**（`<path> --provider codex|kimi`）。认领判定各 provider 自持：解析命令首 token 的 file_name == cc-reporter[.exe] 且余参恰为 `--provider <self>`（同 claude 的严格哲学：不裸 contains，不误伤用户自有 hook）。路径解析沿用两级语义：优先复用该 provider 配置中已存在且文件存在的 reporter 路径（不折腾工作中的配置——本机 dev 路径因此被保留），否则 app 同目录 sidecar。
...
4. `install-hooks.mjs` 不扩展；README 补手动片段；`cc-reporter setup` 子命令为指定后续项。（用户追问后确认）
