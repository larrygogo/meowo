...
**Tech Stack:** Rust（tauri 2、rusqlite、serde_json）+ React 18 + Vite + TypeScript（vitest）。三 crate workspace：`cc-store`（DB）、`cc-reporter`（Agent 抽象）、`cc-app`（Tauri 后端）。
...
- ProviderKey 前后端单一事实源同步点：`api.ts` 的 `ProviderKey` 联合类型、`providers.tsx` 的 `PROVIDERS`、Rust `cc_store::ProviderKey::ALL`（本计划不新增 provider，仅使用）。
- Rust 错误：cc-store 用 `thiserror`（`StoreError`，`?` 直接传播）；异步用 `tauri::async_runtime`。
...
**修改：**
- `crates/cc-reporter/src/agent.rs` — `Agent` trait 增 `launch_args()` + 三 impl + 测试。
- `crates/cc-store/src/store.rs` — 增 `recent_cwds()` + 测试。
...
**Files:**
- Modify: `crates/cc-reporter/src/agent.rs`（trait 定义 `:36` 附近；三 impl `:48-159`；测试 mod `:212`）
...
在 `crates/cc-reporter/src/agent.rs` 的 `mod tests` 末尾（`resume_args_per_provider` 之后）加：
...
Run: `cargo test -p cc-reporter launch_args_per_provider`
...
Run: `cargo test -p cc-app default_agent`
...
**Files:**
- Modify: `crates/cc-store/src/store.rs`（在 `live_session_liveness` `:689` 附近加方法；测试进文件内 `#[cfg(test)]` 或新增）
...
Run: `cargo test -p cc-store recent_cwds`
...
```bash
git add crates/cc-store/src/store.rs
...
Run: `cargo build -p cc-app && cargo test -p cc-app`
...
Run: `cargo test -p cc-app validate_cwd`
...
Run: `cargo test -p cc-app validate_cwd && cargo build -p cc-app`
...
**Interfaces:**
- Consumes: `ccsetup::reporter_path_from_hooks`（已 pub）、`cc_reporter::codex::codex_home`、`cc_reporter::kimi::kimi_share_dir`（均 pub）。
...
Run: `cargo test -p cc-app hooks_check`
Expected: 编译失败（函数未定义）。
...
Run: `cargo test -p cc-app hooks_check && cargo build -p cc-app`
...
git commit -m "feat(new-session): check_provider_hooks 检测三 provider 的 cc-reporter hooks 接入"
```
