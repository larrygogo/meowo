...
**Architecture:** 镜像 `cc-reporter::agent::Agent` 注册表范式：`ProviderAccount` trait + `ALL` 注册表 + `for_provider(ProviderKey)` + enum↔registry 配对单测。通用 `Account`（全 Option + login_label）+ 通用「用量泳道」`ProviderUsage{lanes: Vec<UsageLane>, note}`（`UsageLane.kind` 为 **enum** `UsageKind`，遵循本仓 enum 单一事实源约定）。三家映射进同一泳道结构；查不到一律降级 None，绝不编造、绝不让整卡报错。

**Tech Stack:** Rust（cc-app `account` 模块、cc-reporter 复用 codex/kimi 路径解析）+ React/TS（AccountSection/UsageScreen/api.ts/i18n）。
...
- **路径复用**：codex/kimi 数据根复用 cc-reporter 现有 `codex::codex_home()`（私有→提 pub）与 `kimi::kimi_share_dir()`（私有→提 pub，env `KIMI_SHARE_DIR`），**不在 account 模块重写**（避免 agent.rs 注释告诫的口径漂移）。
- **用量缓存**：`~/.cc-kanban/usage-cache.json` 改 provider 分键 `{claude:{usage,fetched_at}, codex:{...}, kimi:{...}}`；旧扁平格式容错（当 claude 或忽略重拉）。
...
- 验证：`cargo test -p cc-app`（account 纯函数单测）、`node scripts/prepare-sidecar.mjs && cargo check -p cc-app`、`cd app && bun run build && bun run test`。
...
- `crates/cc-reporter/src/codex.rs` / `kimi.rs` —— `codex_home`/`kimi_share_dir` 提 pub。
...
pub fn for_provider(k: cc_store::ProviderKey) -> &'static dyn ProviderAccount; // 遍历 ALL
...
- [ ] Step 6: `cargo test -p cc-app` 全绿（含迁移的 claude 单测 + 新 enum/registry 测）；`node scripts/prepare-sidecar.mjs && cargo check -p cc-app` 通过。
...
- [ ] Step 6: `cd app && bun run build && bun run test` 全绿；`cargo check -p cc-app` 通过。真机视觉确认 claude 账号卡 + 底栏无回归。
...
**Files:** Modify `crates/cc-reporter/src/codex.rs`（`codex_home` 提 pub）；Create/Modify `app/src-tauri/src/account/codex.rs` + mod 注册。

- [ ] Step 1: cc-reporter `codex.rs`：`codex_home()` 改 `pub`。account/mod.rs 加 `decode_jwt_payload(&str)->Option<serde_json::Value>` + 自写 `base64url_decode_nopad(&str)->Option<Vec<u8>>`，各带单测（畸形/缺段→None）。
...
- [ ] Step 5: `cargo test -p cc-app -p cc-reporter` 全绿；`cargo check -p cc-app` 通过。真机（已登录 codex）`bun run tauri dev` 或装机验证 codex 账号卡（email/plan）+ 用量卡（5h/周窗）出现。
...
**Files:** Modify `crates/cc-reporter/src/kimi.rs`（`kimi_share_dir` 提 pub）；Create/Modify `app/src-tauri/src/account/kimi.rs` + mod 注册。

- [ ] Step 1: cc-reporter `kimi.rs`：`kimi_share_dir()` 改 `pub`。
...
- [ ] Step 3: KimiProviderAccount impl：
  ...
  注册进 ALL。
- [ ] Step 4: `cargo test -p cc-app -p cc-reporter` 全绿；`cargo check -p cc-app` 通过。真机（已登录 kimi）验证用量卡（best-effort：能出最好；出不来也不崩、不影响 claude/codex）。
...
- 路径复用 cc-reporter 的 codex_home/kimi_share_dir（提 pub），不重写。
