...
Run: `cargo test -p cc-app settings_defaults_notifications_on`
...
Run: `cargo test -p cc-app waiting_fingerprint_rules`
...
- **类型一致**：`Settings{archive_hide_days,notifications_enabled}`（Rust & TS 一致）、`default_true`、`waiting_fingerprint(bool,&str,i64)->Option<String>`、`should_notify`（复用）、`notified`/`notified_waiting`/`notify_on`/`present` 全程一致；`cc_store::TranscriptInfo{title,error}` 解构与 Task 1 已合并特性的定义一致。✅
