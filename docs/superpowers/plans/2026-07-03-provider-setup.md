...
**Goal:** cc-app 启动时对检测到已安装的 claude/codex/kimi 幂等自动接线（hooks 指向 cc-reporter），消除「开箱即用只对 claude 成立」的缺口。
...
- 合并逻辑保持纯函数（输入 `&mut Value`/`&mut DocumentMut` + 路径字符串，不依赖 Tauri/app 状态）——为后续项 `cc-reporter setup` 跨 crate 迁移铺路。
...
- Consumes: `cc_store::ProviderKey`、现有 `ccsetup.rs` 全部逻辑
...
```rust
//! provider 自动接线：启动时对检测到已安装的 AI CLI 幂等挂上 cc-reporter hooks。
//! 组织仿 account/（trait + 静态注册表）。合并逻辑保持纯函数（不依赖 Tauri/app 状态），
//! 为后续 `cc-reporter setup` 子命令跨 crate 迁移铺路。
pub mod claude;
...
/// app 可执行同目录的 cc-reporter（打包态 sidecar 与 app 放一起）。
...
/// 严格认领带 provider 参数的命令（codex/kimi 形态）：可执行文件名恰为 cc-reporter[.exe]
...
mod tests {
    ...
    fn claim_provider_cmd_strict() {
        // 认领：带引号/裸路径两种形态。
        assert_eq!(
            claim_provider_cmd("\"C:/x/cc-reporter.exe\" --provider codex", "codex").as_deref(),
        ...
        assert!(claim_provider_cmd("C:/x/cc-reporter.exe --provider codex", "kimi").is_none());
        assert!(claim_provider_cmd("\"C:/x/cc-reporter.exe\"", "codex").is_none());
        assert!(claim_provider_cmd("C:/x/cc-reporter.exe --provider codex --v", "codex").is_none());
        assert!(claim_provider_cmd("node cc-reporter-notify.js --provider codex", "codex").is_none());
    }
}
```
...
```rust
            // 无感适配：幂等把 cc-reporter 接入各 AI CLI（claude: hooks+statusLine）。后台跑，失败不影响启动。
...
Run: `cargo test -p cc-app && cargo clippy --workspace -- -D warnings`
...
Run: `cargo test -p cc-app setup::codex -- --nocapture`
...
Run: `cargo test -p cc-app setup::codex && cargo clippy --workspace -- -D warnings`
...
Run: `cargo test -p cc-app setup::codex::tests::codex_hook_hash`
Expected: FAIL（函数未定义）
...
**Interfaces:**
- Consumes: Task 2 的 `ensure_codex_hooks`/`claimed_codex_entries`/`reporter_path_from_codex`、Task 3 的 `codex_hook_hash`/`snake_event`、Task 1 的 `sibling_reporter`/`backup_once`、`cc_reporter::codex::codex_home()`（pub，已存在）、`crate::fsutil::write_atomic`
...
Run: `cargo test -p cc-app setup::codex::tests::ensure_trusted`
Expected: FAIL（函数未定义）
...
Run: `cargo test -p cc-app setup:: && cargo clippy --workspace -- -D warnings`
Expected: 全 PASS
...
cargo test -p cc-app setup::codex::tests::dryrun_codex -- --ignored --nocapture
```
...
**Interfaces:**
- Consumes: `super::claim_provider_cmd`/`sibling_reporter`/`backup_once`、`cc_reporter::kimi::kimi_share_dir()`（pub，已存在）、`crate::fsutil::write_atomic`
...
mod tests {
    ...
    fn ensure_kimi_hooks_adopts_manual_and_updates_stale_path() {
        ...
        let out = doc.to_string();
        assert_eq!(out.matches(r#""C:/app/cc-reporter.exe" --provider kimi"#).count(), 5);
...
Run: `cargo test -p cc-app setup::kimi`
...
```json
{ "hooks": { "SessionStart": [ { "hooks": [ { "type": "command", "command": "\"<cc-reporter绝对路径>\" --provider codex", "timeout": 5 } ] } ] } }
```

首次启动 codex 会提示审查新 hooks，选 **Trust all and continue** 即可（cc-app 自动接线则无此提示——会一并写入信任记录）。
...
[[hooks]]
event = "SessionStart"
command = '"<cc-reporter绝对路径>" --provider kimi'
timeout = 5
```
