<div align="center">
  ...
  <p>哪个在跑、哪个在等你回复、各自做到哪一步——通过 Claude Code hooks 捕获每个会话的事件,<br/>落进本地 SQLite,再用一个半透明、可吸边的 Tauri 小窗口实时呈现。无需切来切去找终端。</p>
...
- 每个 Claude Code 会话一张卡片:项目名、当前标题/动作、todo 进度、连接状态,以及该会话的 **Context 已用百分比**(取自 Claude Code statusline 的准确值,1M 上下文窗口也能正确显示)。
- 状态分类 tab:全部 / 待交互 / 运行中 / 已归档,各带计数;状态指示一眼可辨——运行中(橙色转圈)、待交互(黄)、在线/闲置(绿)、已断开(虚线环)。
...
- 在卡片上直接给会话改名,写入与 Claude Code `/rename` 一致的记录——贴纸和 `claude --resume` 列表同步显示新名字。
- 点卡片右上角按钮把会话收进「已归档」(可随时还原);设置中可让归档条目在 1 / 7 / 30 天后自动隐藏。
...
- Windows 托盘左键打开设置、右键「设置 / 退出」;macOS 为菜单栏图标(左键面板、右键菜单)。开机自启等开关集中在设置页「通用」分区。
...
> **使用安装包时通常无需手动操作**:cc-app 每次启动会自动幂等地把 cc-reporter 接入 `~/.claude/settings.json`——补齐五个 hook 事件,并把 statusLine 包装成 `~/.cc-kanban/statusline.sh` 以获取准确的 Context 百分比(你原有的 statusLine 会被保留并继续生效)。全程先备份、原子写、已正确则一字不改。前提是 `~/.claude/settings.json` 已存在(运行过一次 Claude Code 即会生成),应用不会代为创建。
