//! 通用编解码：base64url、JWT payload、Unix 秒 → ISO 8601、FNV-1a 指纹。
//!
...
/// FNV-1a 64 的偏移基（起始哈希值）。
pub const FNV1A_OFFSET: u64 = 0xcbf2_9ce4_8422_2325;

/// FNV-1a 64：往 `hash` 里累积一段字节，从 [`FNV1A_OFFSET`] 起步、可多段连续喂入。
/// 各处用途都是「同一份内容」的相等指纹（chat 条目去重 id、剪贴板图指纹），
/// 不需要抗碰撞强度。
