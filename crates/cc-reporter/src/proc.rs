...
/// 所以**向上遍历进程树，返回第一个名字含 "claude" 的祖先**（= 会话本体 claude，终端关掉才会退出）。
...
/// 故 Unix 下改用 ps 取 ppid + comm 上溯，返回第一个 comm 含 "claude" 的祖先。
...
pub fn owner_pid() -> Option<u32> {
    ...
    for _ in 0..16 {
        ...
        if ps_comm(pid).is_some_and(|c| c.to_ascii_lowercase().contains("claude")) {
            return Some(pid);
        }
        pid = ps_ppid(pid)?;
    }
