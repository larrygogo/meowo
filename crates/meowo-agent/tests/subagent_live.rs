            ...
            for run in read_subagent_chat(&KIMI_TRANSCRIPT, wire, &call) {
                ...
                for item in &run.items {
                    let kind = match item {
                        ...
                        meowo_agent::transcript::ChatItem::AssistantDelta { .. } => "assistant_delta",
                        meowo_agent::transcript::ChatItem::Reasoning { .. } => "reasoning",
                        meowo_agent::transcript::ChatItem::ReasoningDelta { .. } => "reasoning_delta",
                ...
                eprintln!("  {} / {:?} -> {:?}", &call[..12.min(call.len())], run.label, census);
            }
        }
    }
