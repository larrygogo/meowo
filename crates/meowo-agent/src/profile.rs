...
//!
//! 目录隔离没有这个问题：每个 profile 各写各的目录，谁也覆盖不了谁。代价是 agent **自己的**会话
//! 历史也按 profile 分家（切到工作账号后 `claude --resume` 列的是工作账号的历史）——这反而是诚实的，
//! 不同账号本就该有不同的历史。meowo 自己的看板不受影响，它记在 board.db 里。
//!
//! # 默认 profile 不注入任何东西
//!
