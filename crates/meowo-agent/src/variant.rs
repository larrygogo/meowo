//! 变体（Variant）：同一 agent 的一个版本形态。数据目录 / 配置格式 / 鉴权 / 可执行位置的差异
//! 全部收敛在这一张声明表里；`probe` 命中后产出 [`Installation`]——「这台机器上该 agent 的实况」。

use std::path::{Path, PathBuf};

use crate::{auth::AuthScheme, config::ConfigFormat, id::AgentId};

/// 数据目录的解析规则。
#[derive(Debug, Clone, Copy)]
pub struct DataDirSpec {
    /// 环境变量覆盖名（如 `KIMI_SHARE_DIR`）。设了就优先于候选目录。
    pub env: Option<&'static str>,
    /// 相对 home 的候选目录，按优先级排列。`probe` 取第一个**已存在**的。
    pub candidates: &'static [&'static str],
}

impl DataDirSpec {
    /// env 覆盖值（非空即取，不校验存在）。
    fn env_override(&self) -> Option<PathBuf> {
        let key = self.env?;
        let v = std::env::var(key).ok()?;
        (!v.is_empty()).then(|| PathBuf::from(v))
    }

    /// **已存在**的数据目录：env 覆盖（须是目录）→ 首个存在的候选 → None（＝该变体未安装）。
    pub fn probe(&self, home: &Path) -> Option<PathBuf> {
        self.probe_tagged(home).map(|(d, _)| d)
    }

    /// 同 [`probe`](Self::probe)，并告知命中的是否为 env 覆盖。env 指向的目录**无法**判定属于哪个变体
    /// （没有形态信号），故命中它的变体会把 tag 改成 `"env-override"`，而非谎报成自己的 tag。
    fn probe_tagged(&self, home: &Path) -> Option<(PathBuf, bool)> {
        if let Some(d) = self.env_override() {
            return d.is_dir().then_some((d, true));
        }
        self.candidates.iter().map(|c| home.join(c)).find(|p| p.is_dir()).map(|d| (d, false))
    }

    /// 全新安装**应当**写入的位置：env 覆盖 → 首个候选。不要求存在。
    /// 所有变体都 probe 不中时，由 agent 的首选变体给出这个默认，供「装好之前」的路径展示与写入。
    pub fn default_dir(&self, home: &Path) -> Option<PathBuf> {
        self.env_override().or_else(|| self.candidates.first().map(|c| home.join(c)))
    }
}

/// 可执行文件的查找规则。找不到时调用方回退裸名走 PATH。
#[derive(Debug, Clone, Copy)]
pub struct ExeSpec {
    /// 无扩展名的可执行名（Windows 自动补 `.exe`）。
    pub stem: &'static str,
    /// `data_dir` 内的候选子目录（如 `"bin"`）。
    pub in_data: &'static [&'static str],
    /// home 内的候选子目录（如 `".kimi-code/bin"`）。env 改了 data_dir 时靠这些兜底。
    pub in_home: &'static [&'static str],
}

impl ExeSpec {
    /// 平台化的可执行文件名。
    pub fn file_name(&self) -> String {
        crate::exe_file_name(self.stem)
    }

    /// 逐候选找真实存在的可执行；全不中返回 None。
    pub fn probe(&self, data_dir: &Path, home: Option<&Path>) -> Option<PathBuf> {
        let name = self.file_name();
        let from_data = self.in_data.iter().map(|d| crate::join_rel(data_dir, d).join(&name));
        let from_home = home
            .into_iter()
            .flat_map(|h| self.in_home.iter().map(move |d| crate::join_rel(h, d).join(crate::exe_file_name(self.stem))));
        from_data.chain(from_home).find(|p| p.is_file())
    }
}

/// 环境变量指定数据目录时的变体标签。此时用的是首个变体的 config/auth/exe 规则——env 目录的
/// 版本形态无从判定，故不谎报成 `"modern"`/`"legacy"`，日志里如实标出。
pub const ENV_OVERRIDE_TAG: &str = "env-override";

/// 同一 agent 的一个版本形态——所有版本差异收敛于此。声明为 `const`，进 `&'static [Variant]`。
#[derive(Debug, Clone, Copy)]
pub struct Variant {
    /// 变体标识，仅用于日志/诊断（如 `"modern"` / `"legacy"`）。
    pub tag: &'static str,
    pub data_dir: DataDirSpec,
    pub config: ConfigFormat,
    /// 未登录/无鉴权概念的 agent 为 None。
    pub auth: Option<&'static AuthScheme>,
    pub exe: ExeSpec,
}

impl Variant {
    /// 该变体在本机是否装着；命中则产出实况。
    pub fn probe(&self, id: AgentId, home: &Path) -> Option<Installation> {
        let (data_dir, from_env) = self.data_dir.probe_tagged(home)?;
        let mut inst = self.installation_at(id, data_dir, Some(home));
        if from_env {
            inst.variant_tag = ENV_OVERRIDE_TAG;
        }
        Some(inst)
    }

    /// 以给定 data_dir 构造实况（跳过目录存在性判定）。供 `probe` 与「未安装时的默认位置」共用。
    pub(crate) fn installation_at(&self, id: AgentId, data_dir: PathBuf, home: Option<&Path>) -> Installation {
        let exe = self.exe.probe(&data_dir, home);
        Installation {
            id,
            variant_tag: self.tag,
            data_dir,
            config: self.config,
            auth: self.auth,
            exe,
            exe_stem: self.exe.stem,
        }
    }
}

/// probe 命中后的运行时事实。setup / account / reporter 三条链路都以它为唯一输入，
/// 不再各自重推「kimi 的目录到底是哪个」。
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct Installation {
    pub id: AgentId,
    pub variant_tag: &'static str,
    pub data_dir: PathBuf,
    pub config: ConfigFormat,
    pub auth: Option<&'static AuthScheme>,
    /// 可执行绝对路径；None = 候选位置都没有（此时 `exe_command` 回退裸名走 PATH）。
    pub exe: Option<PathBuf>,
    /// 回退裸名用的可执行名（无扩展名）。
    pub exe_stem: &'static str,
}

impl Installation {
    /// 承载 hooks 的配置文件路径。
    pub fn config_path(&self) -> PathBuf {
        crate::join_rel(&self.data_dir, self.config.config_rel())
    }

    /// 凭据文件路径（该变体无鉴权则 None）。
    pub fn credentials_path(&self) -> Option<PathBuf> {
        self.auth.map(|a| crate::join_rel(&self.data_dir, a.credentials_rel))
    }

    /// 可执行命令串：绝对路径优先（Meowo 拉起的终端 PATH 未必含该 agent，或它是 shim/别名），
    /// 找不到则回退裸名交给 PATH 解析。
    pub fn exe_command(&self) -> String {
        match &self.exe {
            Some(p) => p.to_string_lossy().into_owned(),
            None => self.exe_stem.to_string(),
        }
    }
}

#[cfg(test)]
mod tests {
    use super::*;

    fn tmp(name: &str) -> PathBuf {
        let p = std::env::temp_dir().join(format!("meowo-agent-{name}-{}", std::process::id()));
        let _ = std::fs::remove_dir_all(&p);
        p
    }

    #[test]
    fn env_override_is_tagged_honestly_not_as_first_variant() {
        // KIMI_SHARE_DIR 式覆盖指向的目录无形态信号：不能因为它命中了首个变体就报成 "modern"。
        let home = tmp("envtag");
        let target = home.join("some-copy");
        std::fs::create_dir_all(&target).unwrap();
        let key = "MEOWO_TEST_DATA_DIR";
        std::env::set_var(key, &target);

        let v = Variant {
            tag: "modern",
            data_dir: DataDirSpec { env: Some(key), candidates: &[".modern"] },
            config: ConfigFormat::KimiToml,
            auth: None,
            exe: ExeSpec { stem: "x", in_data: &["bin"], in_home: &[] },
        };
        let inst = v.probe(AgentId::new("t"), &home).expect("env 目录存在应命中");
        assert_eq!(inst.variant_tag, ENV_OVERRIDE_TAG);
        assert_eq!(inst.data_dir, target);

        // env 指向不存在的目录 → 不命中（detect 视为未安装），由 default_dir 兜底。
        std::env::set_var(key, home.join("nope"));
        assert!(v.probe(AgentId::new("t"), &home).is_none());
        assert_eq!(v.data_dir.default_dir(&home), Some(home.join("nope")));

        std::env::remove_var(key);
        let _ = std::fs::remove_dir_all(&home);
    }

    #[test]
    fn data_dir_probe_takes_first_existing_candidate() {
        let home = tmp("probe");
        let spec = DataDirSpec { env: None, candidates: &[".modern", ".legacy"] };

        // 都不存在 → probe None，但 default_dir 给首选（供全新安装写入）。
        assert_eq!(spec.probe(&home), None);
        assert_eq!(spec.default_dir(&home), Some(home.join(".modern")));

        // 只有旧版存在 → 命中旧版。
        std::fs::create_dir_all(home.join(".legacy")).unwrap();
        assert_eq!(spec.probe(&home), Some(home.join(".legacy")));

        // 两者都在 → 首选优先。
        std::fs::create_dir_all(home.join(".modern")).unwrap();
        assert_eq!(spec.probe(&home), Some(home.join(".modern")));

        let _ = std::fs::remove_dir_all(&home);
    }

    #[test]
    fn exe_probe_prefers_data_dir_then_home() {
        let home = tmp("exe");
        let data = home.join(".kimi");
        let spec = ExeSpec { stem: "kimi", in_data: &["bin"], in_home: &[".kimi-code/bin"] };
        let name = spec.file_name();

        std::fs::create_dir_all(&data).unwrap();
        assert_eq!(spec.probe(&data, Some(&home)), None);

        // 仅 home 候选存在 → 用它。
        let home_bin = home.join(".kimi-code").join("bin");
        std::fs::create_dir_all(&home_bin).unwrap();
        std::fs::write(home_bin.join(&name), b"").unwrap();
        assert_eq!(spec.probe(&data, Some(&home)), Some(home_bin.join(&name)));

        // data_dir 候选出现 → 优先。
        let data_bin = data.join("bin");
        std::fs::create_dir_all(&data_bin).unwrap();
        std::fs::write(data_bin.join(&name), b"").unwrap();
        assert_eq!(spec.probe(&data, Some(&home)), Some(data_bin.join(&name)));

        let _ = std::fs::remove_dir_all(&home);
    }
}
