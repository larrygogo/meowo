...
const QA: { q: string; a: React.ReactNode }[] = [
  ...
  {
    q: "同一个工具能挂多个账号吗？",
    a: "可以。在设置里为同一个工具添加多个官方账号，每个账号有独立的登录凭据与会话历史，互不影响。点一下即可切换，配额读数和登录状态会跟着当前账号走。",
  },
  {
    q: "没有官方账号，能用 API 中转吗？",
    a: (
      <>
        可以。在账号设置里按模型接入 API 中转：填入中转地址、模型名和密钥即可启用，模型名可从推荐项选择或直接填中转商提供的任意 ID。官方账号与 API 中转两种接入方式互斥；配置中转期间仍使用官方账号，用中转时官方配额不适用。
      </>
    ),
  },
  {
    q: "贴纸的外观能改吗？",
    a: "能。提供 7 种贴纸配色、扁平与立体两种风格、深 / 浅 / 跟随系统三种主题，还能调整不透明度（配合系统毛玻璃透出桌面）和界面密度。改动实时生效，两个窗口一起套用。",
  },
  {
    q: "支持 Linux 吗？",
