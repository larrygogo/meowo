...
function build(lang: Lang): { head: { eyebrow: string; title: string; lead: string }; qa: QA[]; ctaTitle: string; ctaSub: string } {
  ...
  if (lang === "en") {
    return {
      ...
      qa: [
        ...
        { q: "Can I see the agent's task progress?", a: "Yes. When the agent breaks a task into steps with its todo tool, the chat window shows the list live — the item in progress and the done count refresh as it works, so you don't have to scroll the terminal output. Agents whose todos are incremental events rather than snapshots (e.g. current Claude Code) don't show a list yet." },
  ...
  return {
    ...
    qa: [
      ...
      { q: "能看到 Agent 的任务进度吗？", a: "能。Agent 用待办工具拆解任务后，对话窗口会实时显示这份清单：正在做哪一条、完成了几条都跟着刷新，不用翻终端输出找进度。待办是增量事件而非快照的 Agent（如当前版本的 Claude Code）暂不显示清单。" },
